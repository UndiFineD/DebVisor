...
2. **API Mismatch in Tests**: Tests expect different APIs than implementations
...
3. **Async Test Issues**: `unittest.TestCase` + `@pytest.mark.asyncio` incompatibility
...
### Medium Priority
4. **Weak Fixture Usage**: Tests don't use shared fixtures from conftest
   - Impact: Code duplication
   - Recommendation: Refactor to use conftest fixtures

5. **Poor Test Isolation**: Some session-scoped fixtures may leak state
   - Recommendation: Review fixture scopes

### Low Priority
6. **Enum Collection Warnings**: Source code enums named `TestStatus`, `TestScenario`
...
2. **Review and fix API mismatches** in:
   - `test_api_versioning.py`
   - `test_slo_tracking.py`

3. **Migrate unittest tests to pytest** (gradual):
...
5. **Add test markers** to existing tests:
   ```python
   @pytest.mark.unit
   def test_something():
       ...
