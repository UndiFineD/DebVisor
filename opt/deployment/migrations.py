...
class Migration:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d["status"] = self.status.value  # type: ignore[name-defined]
        d["steps"] = [s.to_dict() for s in self.steps]  # type: ignore[name-defined]
        return d  # type: ignore[name-defined]
    ...
    async def execute_migration(
        ...
        try:
            ...
            for step in migration.steps:
                ...
                if not dry_run:
                    cursor.execute(step.up_sql)  # type: ignore[name-defined]
    ...
    async def rollback_migration(
        ...
        try:
            ...
            for step in reversed(migration.steps):
                logger.info(f"Rolling back: {step.description}")  # type: ignore[name-defined]

                if not dry_run:
                    cursor.execute(step.down_sql)  # type: ignore[name-defined]
            ...
            if self.connection:
                self.connection.rollback()
            logger.error(f"Rollback failed: {e}")  # type: ignore[name-defined]
...
class MigrationManager:
    ...
    async def rollback_migration(self, version: str, dryrun: bool=False) -> tuple[Any, ...]:
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "current_version": current,  # type: ignore[name-defined]
            "applied": applied,
            "pending": pending,
...
def create_phase4_migrations() -> MigrationManager:
    ...
    return manager  # type: ignore[name-defined]
