...
(`host-model`across
nodes) and matching hugepage config.\n\n- Encrypted libvirt migration channel (TLS
certificates per
...
API
requests carry OIDC token -> claims mapped to groups.\n\n- Console tickets: short?lived
signed token
referencing `vm_uuid`+ expiry.\n\n- Panel enforces ownership before exposing VNC/SPICE
websocket
