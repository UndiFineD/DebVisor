...
Grafana
Dashboards\n\n- **dns-dhcp-overview.json**: Real-time DNS/DHCP monitoring with TSIG
rotation
tracking\n\n- **multi-tenant-isolation.json**: Per-tenant zone health with cross-lease
leak
detection\n\n- **compliance-mfa-audit.json**: MFA enforcement status and audit trail
visualization\n\n## Kubernetes Manifests\n\nThese manifests are**test fixtures only**.
They are
intended to\nexercise dashboards and alerting paths in a lab or staging cluster
and\nshould not be
enabled by default in production.\nThe synthetic metrics job is**optional**and intended
primarily
...
the`synthetic-metrics-config`ConfigMap. To\ncustomize them for a given cluster, edit the
values in
