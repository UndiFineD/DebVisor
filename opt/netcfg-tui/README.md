...
Spanning
Tree Protocol (STP) is enabled by default on the bridge\n\n- Adjustable STP timers
(ForwardDelay,
HelloTime, MaxAge)\n\n- Optional bonding (e.g., `bond0`) with common modes (active-backup,
802.3ad,
...
Legacy Curses TUI\n\nRun the legacy TUI (non-privileged). By default, a single bridge
`br0`is
...
192.168.2.254/24\n br-tenant (Tenant)\n +-- eth4 (active)\n +-- eth5 (backup, STP)\n +--
IP:
10.0.0.254/24\nEach bridge isolated, no cross-talk between bridges. VMs connect to
appropriate
...
special
chars - ensure quoted properly\n\n## 3. Interface doesn't support wireless - verify
hardware\n\n##
...
/etc/systemd/network/*.network`\n\n- Verify systemd-networkd is enabled: `systemctl enable
--now
systemd-networkd`\n\n## Roadmap\n\n- [x] Core TUI functionality (interface selection,
DHCP/static
