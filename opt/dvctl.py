...
class DebVisorController:
    ...
    def status(self, component: str = "all"):
        ...
        if component in ["all", "vm"]:
            status_report["components"]["virtualization"] = self._check_service("libvirtd")
    ...
    def upgrade(self, image_path: str):
        ...
        try:
            ...
            logger.info(f"Current Status: Active={status['active_slot']}, Target={status['inactive_slot']}")
    ...
    def harden(self):
        ...
        script_path = os.path.join(os.path.dirname(__file__), "security", "ssh_hardener.py")
    ...
    def discover(self, timeout: int = 5):
        ...
        script_path = os.path.join(os.path.dirname(__file__), "discovery", "zerotouch.py")
        if os.path.exists(script_path):
            subprocess.run([sys.executable, script_path, "scan", "--timeout", str(timeout)])
        else:
...
def main():
    ...
    drift_parser.add_argument("--generate", action="store_true", help="Generate golden manifest from current state")
