...
- `99-debvisor-firstboot-enable.chroot`installs and
enables`debvisor-firstboot.service`.

- Includes under `config/includes.chroot/` are copied into the target
filesystem:
...
On first boot, `debvisor-firstboot.sh` runs once (via the systemd unit),
performs
initial provisioning according to `/etc/debvisor-profile`, and writes the
profile
