...
class ThemePreference(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_theme_preference_user_id', 'user_id'),
    )
...
BatchOperation.__table_args__ += (
    CheckConstraint("status IN ('pending', 'queued', 'running', 'completed', 'failed', 'rolled_back', 'cancelled')"),
