...
conventions for all DebVisor Grafana dashboards. Adherence to these standards
ensures
consistent
user experience, maintainability, and reliable monitoring across the
platform.\n\n###
Goals\n\n-
Provide intuitive, discoverable dashboards for operators of all skill
levels\n\n- Enable
consistent
visualization across heterogeneous environments\n\n- Simplify dashboard
maintenance and
evolution\n\n- Reduce cognitive load for operators using multiple
dashboards\n\n## Naming
Standards\n\n### Dashboard Naming Convention\n\nAll dashboard names MUST follow
this
format:\n\n-
...
Convention\n\nPanel titles MUST be:\n\n- **CamelCase:**"CPU Usage", "Query
Latency", "OSD
Health"\n\n- **Descriptive:**Indicates what metric is shown\n\n-
**Unit-aware:**Include
units in
title or legend\n\n- **Role-appropriate:**Clear for target audience (operator,
architect,
dev)\n\n### Examples [2]\n\n {\n "title": "CPU Usage (%)",\n "targets": [\n
{"expr":
...
$pod_namespace,
$osd_filter,
$alert_severity\n ? Wrong: $pod-namespace (hyphens), $POD_NAMESPACE (caps),
$_pod (leading
underscore)\n\n## Color Standards\n\n### Primary Palette\n\nAll dashboards MUST
use this
standard
...
|\n\n###
Multi-Series Coloring\n\nFor dashboards with many series (e.g., per-node
metrics):\n
Automatic
...
colors for
important
series\n\n- Avoid color-blind confusing combinations (red-green at 1:1
ratio)\n\n- Legend
should
...
stat panels, each 3 cols wide)\n Row 2: Primary Metrics (1-2 large time series
panels)\n
Row 3:
Secondary Metrics (2-4 medium panels)\n Row 4: Detailed Analysis (Large table,
heatmap, or
node
...
**Top:**Minimal; use
only when
space is tight\n\n### Legend Values\n\n- **Always include:**Current value (for
operators
to quickly
...
performance
degradation above 85% |\n| Network Utilization | 75% | 90% | 10G links usually have 100ms |\n\n### Service
...
locally\n\n- Cloud
DNS response times are 20-50ms\n\n- >100ms indicates resolver performance
issue\n\n-
...
time\nmax_samples:
1000000 # Max time series returned\nmax_data_points: 10000 # Max points
plotted\n\n### For
Large
Dashboards\n\n- Limit panels to yellow->red)\n\n- []**Variables:**Test
all
template variable combinations\n\n- []**Drill-down:**Test navigation links\n\n-
[]**Performance:**Dashboard loads in  /tmp/test.json\n\n## Migration &
...
"lastSavedBy":
"ops-team"\n
}\n}\n\n### Version Bumping\n\n- Major (v1.0 -> v2.0): Breaking changes (renamed
panels,
new
...
between
environments:\n\n## Export from staging\n\ncurl -s
[http://grafana-staging:3000/api/dashboards/uid/overview]([http://grafana-staging:3000/api/dashboards/uid/overvie]([http://grafana-staging:3000/api/dashboards/uid/overvi]([http://grafana-staging:3000/api/dashboards/uid/overv]([http://grafana-staging:3000/api/dashboards/uid/over]([http://grafana-staging:3000/api/dashboards/uid/ove]([http://grafana-staging:3000/api/dashboards/uid/ov]([http://grafana-staging:3000/api/dashboards/uid/o]([http://grafana-staging:3000/api/dashboards/uid/]([http://grafana-staging:3000/api/dashboards/uid]([http://grafana-staging:3000/api/dashboards/ui]([http://grafana-staging:3000/api/dashboards/u]([http://grafana-staging:3000/api/dashboards/]([http://grafana-staging:3000/api/dashboards]([http://grafana-staging:3000/api/dashboard]([http://grafana-staging:3000/api/dashboar]([http://grafana-staging:3000/api/dashboa]([http://grafana-staging:3000/api/dashbo]([http://grafana-staging:3000/api/dashb]([http://grafana-staging:3000/api/dash]([http://grafana-staging:3000/api/das]([http://grafana-staging:3000/api/da]([http://grafana-staging:3000/api/d]([http://grafana-staging:3000/api/]([http://grafana-staging:3000/api]([http://grafana-staging:3000/ap]([http://grafana-staging:3000/a]([http://grafana-staging:3000/]([http://grafana-staging:3000]([http://grafana-staging:300]([http://grafana-staging:30]([http://grafana-staging:3]([http://grafana-staging:]([http://grafana-staging]([http://grafana-stagin]([http://grafana-stagi]([http://grafana-stag]([http://grafana-sta]([http://grafana-st]([http://grafana-s]([http://grafana-]([http://grafana]([http://grafan]([http://grafa]([http://graf]([http://gra]([http://gr](http://gr)a)f)a)n)a)-)s)t)a)g)i)n)g):)3)0)0)0)/)a)p)i)/)d)a)s)h)b)o)a)r)d)s)/)u)i)d)/)o)v)e)r)v)i)e)w)
\\n\n- H "Authorization: Bearer $TOKEN" > overview.json\n\n## Adjust
environment-specific
values\n\njq '.dashboard.panels[].targets[].expr |= gsub("staging"; "prod")'
overview.json\n\n##
Import to production\n\ncurl -X POST
[http://grafana-prod:3000/api/dashboards/db]([http://grafana-prod:3000/api/dashboards/d]([http://grafana-prod:3000/api/dashboards/]([http://grafana-prod:3000/api/dashboards]([http://grafana-prod:3000/api/dashboard]([http://grafana-prod:3000/api/dashboar]([http://grafana-prod:3000/api/dashboa]([http://grafana-prod:3000/api/dashbo]([http://grafana-prod:3000/api/dashb]([http://grafana-prod:3000/api/dash]([http://grafana-prod:3000/api/das]([http://grafana-prod:3000/api/da]([http://grafana-prod:3000/api/d]([http://grafana-prod:3000/api/]([http://grafana-prod:3000/api]([http://grafana-prod:3000/ap]([http://grafana-prod:3000/a]([http://grafana-prod:3000/]([http://grafana-prod:3000]([http://grafana-prod:300]([http://grafana-prod:30]([http://grafana-prod:3]([http://grafana-prod:]([http://grafana-prod]([http://grafana-pro]([http://grafana-pr]([http://grafana-p]([http://grafana-]([http://grafana]([http://grafan]([http://grafa]([http://graf]([http://gra]([http://gr]([http://g](http://g)r)a)f)a)n)a)-)p)r)o)d):)3)0)0)0)/)a)p)i)/)d)a)s)h)b)o)a)r)d)s)/)d)b)
\\n\n- H "Authorization: Bearer $TOKEN" \\n\n- d @overview.json\n\n## Governance
&
Maintenance\n\n### Dashboard Lifecycle\n\nNew Dashboard (Community
Contribution)\n v\nCode
Review
(Dashboard Standards Compliance)\n v\nTesting (Queries, Thresholds,
Performance)\n
v\nApproval
...
documentation
current\n1.**Feedback:**Gather operator feedback and
iterate\n1.**Incidents:**Use
dashboard data to
investigate incidents\n1.**Deprecation:**Archive or remove unused
dashboards\n\n###
Contributing New
Dashboards\n\nProcess for contributing new dashboards:\n1.**Design
Phase:**Create
dashboard JSON,
test locally\n1.**Documentation:**Add to opt/grafana/dashboards/
directory\n1.**PR
Submission:**Submit dashboard JSON + README entry\n1.**Code Review:**Verify
naming,
standards
compliance, queries\n1.**Approval:**Dashboard owner or lead
approves\n1.**Merge:**Merged
to main
