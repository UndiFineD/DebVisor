...
-__Traefik reverse proxy__(`traefik/traefik-compose.yml`):

- Fronts HTTP/S services and can terminate TLS for containers

    running on the local Docker daemon.

-__GitLab Runner__(`gitlab-runner/gitlab-runner-compose.yml`):

- Provides CI/CD runner capacity using the Docker executor.

-__Other examples__: Additional small services intended as reference
  deployments.
