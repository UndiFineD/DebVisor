...
class AnomalyCLI:
    ...
    def _add_baseline_commands(self, subparsers) -> None:
        ...
        establish.add_argument("--percentile", action="store_true", help="Use percentile-based baseline")
    ...
    def _add_trend_commands(self, subparsers) -> None:
        ...
        analyze.add_argument("--hours", type=int, default=24, help="Analysis window hours (default: 24)")
        ...
        if args.baseline_cmd == "establish":
            try:
                ...
                if baseline:
                    ...
                    print(f"Failed to establish baseline (insufficient data)")
                    return 1
            except KeyError:
                print(f"Unknown metric type: {args.metric_type}", file=sys.stderr)
                return 1
    ...
    def _handle_alert(self, args) -> int:
        ...
        if args.alert_cmd == "list":
            ...
            if alerts:
                ...
                print(tabulate(
                    alert_data,
                    headers=["Alert ID", "Resource", "Metric", "Type", "Severity", "Confidence", "Value"],
                    tablefmt="grid"
                ))
            else:
    ...
    def _handle_trend(self, args) -> int:
        ...
        if args.trend_cmd == "analyze":
            ...
            if trend_data:
                print(tabulate(
                    trend_data,
                    headers=["Resource", "Metric", "Direction", "Strength", "Change/Hour", "Confidence"],
                    tablefmt="grid"
                ))
            else:
