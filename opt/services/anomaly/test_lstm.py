    ...
    # Generate sine wave data
        ...
        for i in range(100):
            ...
            self.engine.add_metric(
                self.resource_id,
                self.metric_type,
                val,  # type: ignore[name-defined]
                base_time + timedelta(minutes=i * 5),  # type: ignore[name-defined]
            )

        # Train model
        print("Training LSTM model...")
        _success=self.engine.train_model(self.resource_id, self.metric_type)
        self.assertTrue(success, "Model training failed")  # type: ignore[name-defined]

        # Verify model exists and is trained
        _key=(self.resource_id, self.metric_type)
        self.assertIn(key, self.engine.lstm_models)  # type: ignore[name-defined]
        self.assertTrue(self.engine.lstm_models[key].is_trained)  # type: ignore[name-defined]

        # Test normal value (should not be anomaly)
        _next_val=50 + 10 * np.sin(100 * 0.2)
        alerts=self.engine.detect_anomalies(
            self.resource_id, self.metric_type, next_val, methods=[DetectionMethod.LSTM]  # type: ignore[name-defined]
        )
