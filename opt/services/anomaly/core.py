...
class AnomalyDetectionEngine:
    ...
    def _detect_zscore_anomaly(
        ...
        if z_score > self.z_score_threshold:
            ...
            return AnomalyAlert(
                ...
                message=f"{anomaly_type.value} detected: {current_value:.2f} (Z-score: {z_score:.2f})",
                details={"z_score": z_score, "baseline_mean": baseline.mean, "baseline_stddev": baseline.stddev}
            )
    ...
    def _detect_lstm_anomaly(
        ...
        if deviation > threshold:
            ...
            return AnomalyAlert(
                ...
                message=f"{anomaly_type.value} detected (LSTM): {current_value:.2f} (Pred: {predicted_value:.2f})",
                details={"predicted": predicted_value, "deviation": deviation, "threshold": threshold}
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alert_ack_rate": len([a for a in self.alerts if a.acknowledged]) / len(self.alerts) if self.alerts else 0
        }
