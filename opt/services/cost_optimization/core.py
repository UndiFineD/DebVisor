...
class CostOptimizer:
    ...
    def analyze_resource_usage(self, resources: List[Dict[str, Any]]) -> List[OptimizationRecommendation]:
        """
        ...
        for res in resources:
            if res["type"] == "vm":
                ...
                if res["metrics"].get("cpu_avg", 0) < 5 and res["metrics"].get("network_io", 0) < 100:
                    savings = self._calculate_monthly_cost(res["specs"])
                    rec = OptimizationRecommendation(
                        resource_id=res["id"],
                        resource_type="vm",
                        recommendation_type="idle",
                        description=f"VM {res['id']} appears idle (CPU < 5%). Consider terminating or stopping.",
                ...
                # Check for over-provisioning
                    ...
                    rec = OptimizationRecommendation(
                        ...
                        description=f"VM {res['id']} is underutilized (CPU < 20%). Resize from {res['specs']['cpu']}vCPU/{res['specs']['memory_gb']}GB to {new_specs['cpu']}vCPU/{new_specs['memory_gb']}GB.",
                        estimated_savings_monthly=savings,
