...
class TargetHostSelector:
    ...
    def _score_host(
        ...
        memory_score = min(100, (metrics.ram_free_mb - required_memory) / max(required_memory, 1) * 50 + 50)
...
class AdvancedMigrationManager:
    ...
    def execute_post_copy(self, plan: MigrationPlan) -> bool:
        ...
        logger.info(f"Executing {plan.strategy.value} migration: {plan.vm_id} -> {plan.target_host}")
