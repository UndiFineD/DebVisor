...
class ReplicationScheduler:
    ...
    def get_policy(self, policy_id: str) -> Optional[ReplicationPolicy]:
        """Get policy by ID."""
    ...
    async def execute_job(self, job: ReplicationJob) -> bool:
        ...
        try:
            ...
            if job.sync_type == SyncType.FULL:
                ...
                since = last_job.completed_at if last_job else datetime.min.replace(tzinfo=timezone.utc)
    ...
    def _get_last_successful_job(self, policy_id: str, 
        ...
        if jobs:
            return max(jobs, key=lambda j: j.completed_at or datetime.min.replace(tzinfo=timezone.utc))
        return None
