        ...
        try:
            ...
            if status:
                try:
                    ...
                    return self._error_response(f"Invalid status: {status}", "validation_error", 400)
        ...
        try:
            ...
            for rt in request_data["resource_types"]:
                try:
                    ...
                    return self._error_response(f"Invalid resource type: {rt}", "validation_error", 400)
            ...
            self.logger.info(f"Replication setup: {config.source_region_id} -> {config.target_region_id}")
            return self._json_response(
                data=config.to_dict(),
