...
class XenVMManager:
    ...
    async def create_vm(
        ...
        try:
            # Generate Xen config file
            config_path = f"/tmp/xen-{name}.cfg"
            config_content = self._generate_vm_config(
                name, vcpus, memory_mb, vm_type, disk_path, network_config
            )
            ...
            logger.info(f"Created Xen VM: {name}")
