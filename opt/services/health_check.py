...
class BinaryChecker:
    ...
    def check(self) -> CheckResult:
        ...
        return CheckResult(
            ...
            status=status,
            score=score,
            message=f"{len(self.REQUIRED_BINARIES) - len(missing)}/{len(self.REQUIRED_BINARIES)} binaries available",
            details={"missing_binaries": missing},
            remediation="Install missing packages: " + ", ".join(missing) if missing else None,
        )
...
class ServiceChecker:
    ...
    def check(self) -> CheckResult:
        ...
        return CheckResult(
            ...
            status=status,
            score=score,
            message=f"{len(self.REQUIRED_SERVICES) - len(failed)}/{len(self.REQUIRED_SERVICES)} services running",
            details={"failed_services": failed},
            remediation="Restart failed services: systemctl restart " + " ".join(failed) if failed else None,
        )
    ...
    def check(self) -> CheckResult:
        ...
        for name, endpoint in self.ENDPOINTS:
            if endpoint.startswith("/"):
                ...
                try:
                    subprocess.run(
                        ["bash", "-c", f"timeout 2 bash -c 'echo >/dev/tcp/{host}/{port}'"],
                        check=True,
                        capture_output=True,
                        timeout=5
                    )
        ...
        return CheckResult(
            ...
            status=status,
            score=score,
            message=f"{len(self.ENDPOINTS) - len(failed)}/{len(self.ENDPOINTS)} endpoints reachable",
            details={"unreachable_endpoints": failed},
            remediation="Check firewall and service status for: " + ", ".join(failed) if failed else None,
        )
...
class ConfigurationChecker:
    ...
    def check(self) -> CheckResult:
        ...
        return CheckResult(
            ...
            status=status,
            score=score,
            message=f"{len(self.CONFIG_FILES) - len(missing) - len(invalid)}/{len(self.CONFIG_FILES)} configs valid",
            details=details,
...
class HealthCheckFramework:
    ...
    def format_report_table(self, report: HealthReport) -> str:
        ...
        lines = [
            f"\n{'='*70}",
            f"  DebVisor Health Check Report",
