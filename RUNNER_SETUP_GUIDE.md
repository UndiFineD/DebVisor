...
2. **GitHub Actions Runner** (already downloaded to `C:\actions-runner`)
...
4. Ensure this order (move if needed):
...
5. Click OK, then restart the runner service:
...
3. Verify token hasn't expired (1-hour validity):
...
2. Replace `apt-get install jq` with:
...
3. For unavoidable Linux requirements, consider:
...
2. **Runner labels mismatch**: Workflow uses `runs-on: ubuntu-latest` instead of `runs-on: self-hosted`

3. **Multiple queues**: Check if jobs are waiting for hosted runners (billing issue)
...
3. If missing, runner may need reconfiguration or GitHub Actions environment issue
...
2. ✅ Fix PATH to prioritize Git Bash (follow [PATH Configuration](#path-configuration))
3. ✅ Run smoke test workflow (see [Verification](#verification))
4. ✅ Review workflow audit results (check for Linux-only commands)
5. ✅ Update workflows to use `scripts/setup-runner-tools.sh`
