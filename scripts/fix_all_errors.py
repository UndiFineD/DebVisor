    ...
    def _auto_fix_missing_test_imports(self, testdir: Path, stats: RunStats) -> None:
        ...
        for path in testdir.rglob("test_*.py"):
            try:
                ...
                need_unittest = re.search(
                    r"\bTestCase\b|\bunittest\b", content) and not re.search(
                    r"^\s*import\s+unittest\b", content, re.MULTILINE)
                need_pytest = re.search(r"\bpytest\b", content) and not re.search(r"^\s*import\s+pytest\b", content,
                                                                                  re.MULTILINE)
                need_patch = re.search(r"\bpatch\b", content) and not re.search(
                    r"^\s*from\s+unittest\.mock\s+import\s+patch\b", content, re.MULTILINE)
                need_datetime = re.search(
                    r"\bdatetime\b",
                    content) and not re.search(
                    r"^\s*from\s+datetime\s+import\s+datetime\b|^\s*import\s+datetime\b",
                    content,
                    re.MULTILINE)
...
class CI_E115ExpectedIndentationFixer(BaseFixer):
    ...
    E115 occurs when a comment is not indented at least as much as expected
    after a block starter like 'if:', 'def:', 'class:', 'try:', etc.
    """
    ...
    def fix_file(self, path: Path, stats: RunStats):
        ...
        try:
            ...
            # Track expected indentation levels
            for i in range(len(lines)):
                line = lines[i]
                stripped = line.lstrip()
                ...
                # Look backwards for context to determine expected indentation
                expected_indent = self._get_expected_indent(lines, i)

                if expected_indent is not None and curr_indent < expected_indent:
    ...
    def _get_expected_indent(self, lines: List[str], comment_index: int) -> Optional[int]:
        """Determine the expected indentation for a comment at the given index."""
        # Look backwards for the most recent non-empty, non-comment line
        for j in range(comment_index - 1, -1, -1):
            ...
            # If previous line ends with colon, comment needs +4 indent
            if prev_stripped.rstrip().endswith(':'):
                return prev_indent + 4

            # Otherwise, comment should match the indentation of surrounding code
            # Look at the next non-empty, non-comment line
            for k in range(comment_index + 1, len(lines)):
                next_line = lines[k]
...
class CI_E116UnexpectedIndentationFixer(BaseFixer):
    ...
    E116 occurs when a comment is indented more than expected,
    typically when it's over-indented compared to surrounding code.
    """
    ...
    def fix_file(self, filepath: Path, stats: RunStats):
        ...
        try:
            ...
            for i in range(len(lines)):
                ...
                # Find the expected indentation from context
                expected_indent = self._get_context_indent(lines, i)
    ...
    def _get_context_indent(self, lines: List[str], comment_index: int) -> Optional[int]:
        """Get the expected indentation based on surrounding code context."""
        # Look at lines before and after to determine proper indentation
        ...
        for j in range(comment_index - 1, -1, -1):
            ...
            if prev_stripped and not prev_stripped.startswith('#'):
                ...
                # If it ends with colon, comment belongs inside the block
                if prev_stripped.rstrip().endswith(':'):
                    return prev_code_indent + 4
                break
        ...
        # Return the indentation of surrounding code
        if next_code_indent is not None:
            return next_code_indent
        if prev_code_indent is not None:
            return prev_code_indent
...
class E251UnexpectedSpacesFixer(BaseFixer):
    ...
    E251 occurs in function definitions and calls when there are spaces
    around the = sign for keyword arguments, e.g.:
        def foo(param = value):  # E251
        foo(param = value)       # E251
    Should be:
        def foo(param = value):
        foo(param = value)
    """
    ...
    def fix_file(self, path: Path, stats: RunStats):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                # Skip lines that don't have an = in them
                if ' = ' not in line:
                    continue

                new_line = self._fix_keyword_args(line)
    ...
    def _fix_keyword_args(self, line: str) -> str:
        """Fix keyword argument spacing in a line.

        Only fixes = signs that appear to be in function calls or definitions,
        not regular variable assignments.
        """
        # Find all occurrences of ' = ' (space around equals)
        # that are inside parentheses (function calls/definitions)
        result = []
        paren_depth = 0
        i = 0
        ...
        while i < len(line):
            ...
            if char == '(':
                ...
                if is_space_equals:
                    ...
                    result.append(' = ')
                    i += 1  # Skip the  =
                    # Skip space after = if followed by non-space
...
class F821UndefinedNameFixer(BaseFixer):
    ...
    def fix_file(self, path: Path, stats: RunStats):
        ...
        try:
            ...
            # First pass: find all _var assignments and their usages
            underscore_vars = {}
            for i, line in enumerate(lines):
                # Find _var = assignments
                match = re.search(r'\b_([a-zA-Z_]\w*)\s* = \s*', line)
                if match and not line.strip().startswith('#'):
                    var_name = match.group(1)
                    underscore_vars[f"_{var_name}"] = var_name

            # Second pass: fix assignments and usages
            for i, line in enumerate(lines):
                if line.strip().startswith('#'):
                    continue
                ...
                # Fix: _var = value -> var = value
                for underscore_var, base_var in underscore_vars.items():
                    # Replace the assignment
                    new_line = re.sub(
                        rf'\b{re.escape(underscore_var)}\s* = ',
                        f'{base_var}  = ',
                        new_line
                    )
    ...
    def fix_file(self, path: Path, stats: RunStats):
        ...
        try:
            ...
            while i < len(lines):
                ...
                if line.strip().startswith('@'):
                    # Look ahead for blank lines between decorator and def/class
                    j = i + 1
                    while j < len(lines) and not lines[j].strip():
                        j += 1

                    # If next non-blank line is def/class or another decorator
                    if j < len(lines):
                        next_line = lines[j].strip()
                        if next_line.startswith(('def ', 'async def ', 'class ', '@')):
                            # Skip blank lines (don't add them)
                            blank_count = j - i - 1
                            if blank_count > 0:
                                fixed_count += blank_count
                            i = j - 1  # Will be incremented at end of loop

                i += 1

            if fixed_count > 0 and self.apply:
                # Rebuild without blank lines after decorators
                result_lines = []
                i = 0
                while i < len(lines):
                    line = lines[i]
...
class F401UnusedImportFixer(BaseFixer):
    """Fix F401 - module imported but unused.

    F401 occurs when a module is imported but never used in the file.
    This fixer removes the unused import line.
    """

    def run(self, stats: RunStats):
        """Fix F401 by removing unused imports."""
        for path in self.root.rglob("*.py"):
            if not self.should_skip(path):
                self.fix_file(path, stats)
    ...
    def fix_file(self, path: Path, stats: RunStats):
        ...
        try:
            ...
            # Find all imports and check if they're used
            ...
            for i, line in enumerate(lines):
                ...
                if stripped.startswith('import ') or stripped.startswith('from '):
                    imported_names = self._get_imported_names(stripped)

                    for name in imported_names:
                        if not self._is_import_used(content, name, i, lines):
                            # Mark for removal (but be conservative)
    ...
    def _get_imported_names(self, import_line: str) -> List[str]:
        """Extract the imported names from an import line."""
        names = []
    ...
    def _is_import_used(self, content: str, name: str, import_line: int,
                        lines: List[str]) -> bool:
        """Check if an imported name is used in the file."""
        for i, line in enumerate(lines):
            if i == import_line:
                continue
            # Skip comments
            if line.strip().startswith('#'):
                continue
            # Check if name appears as a word boundary
...
class F841UnusedVariableFixer(BaseFixer):
    ...
    F841 occurs when a variable is assigned but never used. Common fixes:
    1. For intentionally unused vars (like loop vars), prefix with _
    2. For vars that should be used, the code needs manual review
    """

    def run(self, stats: RunStats):
        """Fix F841 by prefixing unused variables with underscore."""
        for path in self.root.rglob("*.py"):
            if not self.should_skip(path):
                self.fix_file(path, stats)
    ...
    def fix_file(self, path: Path, stats: RunStats):
        ...
        try:
            ...
            # Pattern: var = value where var is never referenced elsewhere
            for i, line in enumerate(lines):
                if line.strip().startswith('#'):
                    continue

                # Check for simple assignments that might be unused
                ...
                if match:
                    ...
                    is_used = self._is_var_used(content, var_name, i, lines)

                    if not is_used:
                        # Prefix with underscore to indicate intentionally unused
                        lines[i] = f"{indent}_{var_name} = {value}"
                        fixed_count += 1
    ...
    def _is_var_used(self, content: str, var_name: str, assign_line: int,
                     lines: List[str]) -> bool:
        """Check if a variable is used after its assignment."""
        # Look for usages after the assignment line
        for i in range(assign_line + 1, len(lines)):
            line = lines[i]
            # Skip comments
            if line.strip().startswith('#'):
                continue
            # Check if var_name appears as a word (not part of another name)
            # Use word boundary matching
            if re.search(rf'\b{re.escape(var_name)}\b', line):
                # Make sure it's not just another assignment
...
def main():
    ...
    fixers = [
        # Core fixers for fundamental issues (flake8 errors)
        ...
        F401UnusedImportFixer(root, args.apply, target_paths),
