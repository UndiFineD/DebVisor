...
that limits the amount of files we will fix in a run limited to regardless of the amount of files overal selected. for example --max-files=20 will only do the first 20 Codefile it sees in the directory selected.
...
that reads a specified --context Contextfile Codefile.description.md and returns this context into previous-context
...
that reads a specified --context file Codefile.changes.md and returns this context into previous-context
...
that reads a specified --context file Codefile.errors.md and returns this context into previous-context
...
that reads a specified --context file  Codefile.improvements.md and returns this context into previous-context
...
make agent-tests.py
that reads a specified --context file Codefile.tests.py and returns this context into previous-context

then the agent-tests.py will wait for --prompt
it can use standard tools, or create its own tests in python Codefile.tests.py , as long as each line of Codefile is tested by at least one test
