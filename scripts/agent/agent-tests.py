...
class TestsAgent(BaseAgent):
    ...
    def suggest_tests_for_gap(self, gap: CoverageGap) -> str:
        ...
        return (
            f"# Suggested test for {gap.file_path} lines {gap.line_start}-{gap.line_end}\n"
            f"def test_{gap.file_path.replace('/', '_').replace('.py', '')}_line{gap.line_start}():\n"
            f"    # TODO: Add test for {gap.coverage_type.value} coverage\n"
