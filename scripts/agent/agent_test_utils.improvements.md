...
1. **Hyphenated filenames**:
    `agent-changes.py`, `agent-errors.py` are NOT valid Python identifiers
2. **Standalone scripts**:
    These are CLI tools, not a traditional package
3. **Context manager provides safety**:
    `agent_dir_on_path()` properly cleans up `sys.path`
4. **Widely used**:
