...
live-image-amd64.hybrid.iso\n\nThe build script runs`build/sync-addons-playbook.sh`before creating
the ISO so\n`bootstrap-addons.yml`in`config/includes.chroot`stays in sync with the source\nAnsible
playbook.\n\n## Developer Tasks (Docs/Lint)\n\n- Activate the virtual environment, then run linters
...
`/etc/debvisor-addons.d/*.conf`\n- First boot runs `bootstrap-addons.yml`locally to apply addon
