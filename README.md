...
Multi-cluster
management with
failover and federation\n- Advanced network configuration via `netcfg-tui`with
rollback
support\n-
...
runs`build/sync-addons-playbook.sh`before
creating
the ISO so\n`bootstrap-addons.yml`in`config/includes.chroot`stays in sync with
the
source\nAnsible
...
pool for VM
disks\n\n-
zfs: Non-OS disks aggregated into ZFS pool`tank`; datasets for VM, Docker,
and\n\n
K8s\n\n- mixed:
...
`/etc/debvisor-addons.d/*.conf`\n- First boot runs `bootstrap-addons.yml`locally
to apply
addon
roles\n\n## Dry Run\n\nRun`debvisor-firstboot.sh --dry-run`to log intended
actions without
