...
implements comprehensive software supply chain security following industry best practices
including
SLSA Build Level 3, dual-format SBOM generation, cryptographic attestations, policy
enforcement, and
...
artifacts:\n\n```bash\n\n- *Features**:\n\n- Passphrase support via `--pinentry-mode
loopback`\n\n-
Automatic public key import for verification\n\n- Smoke test verification
post-signing\n\n-
--\n\n### 2. Cryptographic Checksums (2)\n\n- *Implementation**:
`.github/workflows/release.yml`(job:`build-artifacts`)\n\nSHA256 checksums generated for
all
artifacts:\n\n```bash\n\n- *Features**:\n\n- Passphrase support via `--pinentry-mode
loopback`\n\n-
Automatic public key import for verification\n\n- Smoke test verification
...
msg :=
sprintf("SBOM must contain at least 10 components, found %d",
...
input.components[*]\n not component.licenses\n msg := sprintf("Component '%s' missing
license
...
license
information", [component.name])\n}\n```text\n## Minimum component count\n\ndeny[msg] {\n
count(input.components) < 10\n msg := sprintf("SBOM must contain at least 10 components,
found %d",
...
component.licenses\n
msg := sprintf("Component '%s' missing license information",
[component.name])\n}\n```text\n\n-
*Execution**:\n\n```bash\n\n- *Execution**:\n\n```bash\n\n- *Execution**:\n\n```bash\n\n-
*Execution**:\n\n```bash\n\n- *Execution**:\n\n```bash\n\n- *Execution**:\n\n```bash\n\n-
...
(6)\n\necho
"Artifact Integrity: $(sha256sum -c *.sha256 && echo PASS)"\necho "GPG Signatures: $(gpg
--verify
*.asc && echo PASS)"\necho "Rekor Inclusion: $(rekor-cli verify --artifact *.tar.gz &&
echo
PASS)"\n```text\ngh run download <run-id> --name sbom-attestation-1.0.0\n\n## Compile
verification
results (7)\n\necho "Artifact Integrity: $(sha256sum -c *.sha256 && echo PASS)"\necho "GPG
