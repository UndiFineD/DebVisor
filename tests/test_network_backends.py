...
class TestVirtualNetworkManagement:
    ...
    async def test_list_networks(self, mock_network_backend):
        ...
        networks = [
            VirtualNetwork(f"net-{i}", f"network-{i}", NetworkType.BRIDGE, 
                         f"192.168.{i}.0/24", NetworkState.ACTIVE, time.time())
            for i in range(3)
        ]
...
class TestNetworkMonitoring:
    ...
    async def test_packet_capture(self, mock_network_backend):
        """Test packet capture"""
        mock_network_backend.start_packet_capture = AsyncMock(return_value="capture-001")
