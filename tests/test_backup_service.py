    ...
    def test_rolling_hash_simulation(self, sample_data_blocks):
        ...
        for block in sample_data_blocks:
            ...
            for i in range(len(block) - window_size + 1):
                ...
                fp = sum(b * (31 ** (window_size - j - 1)) for j, b in enumerate(window))
...
class TestDeduplicationStore:
    ...
    def test_deduplication_ratio_calculation(self, in_memory_chunk_store, sample_data_blocks):
...
class TestSnapshotManagement:
    ...
    def test_snapshot_sorting_by_date(self, sample_snapshots):
        ...
        sorted_snaps = sorted(sample_snapshots, key=lambda s: s.created_at, reverse=True)
...
class TestRetentionPolicies:
    ...
    def test_daily_retention(self, sample_snapshots, retention_policy):
        ...
        to_keep = sorted(daily_snaps, key=lambda s: s.created_at, reverse=True)[:daily_keep]
...
class TestBackupOperations:
    ...
    def test_incremental_backup_simulation(self, in_memory_chunk_store, temp_backup_dir):
        ...
        chunks_after_first = in_memory_chunk_store.total_chunks
        ...
        overlap = set(first_chunks) & set(second_chunks)
