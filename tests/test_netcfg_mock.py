...
class TestMockNetworkBackend:
    ...
    def test_create_bond(self):
        ...
        result = self.backend.create_bond("bond0", ["eth0", "eth1"], mode="active-backup")
...
class TestWiFiOperations:
    ...
    def test_connect_wifi_secured(self):
        ...
        if secured:
            ...
            result = self.backend.connect_wifi("wlan0", secured["ssid"], password="secret123")
            assert result is True
...
class TestRoutingOperations:
    ...
    def test_add_route(self):
        ...
        # Verify route exists
        routes = self.backend.get_routes()
        new_route = next((r for r in routes if r["destination"] == "10.10.0.0/16"), None)
