...
class TestVNCSessionManagement:
    ...
    async def test_list_active_sessions(self, mock_vnc_manager):
        ...
        sessions = [
            VNCSession(f"session-{i}", f"vm-{i}", f"user-{i}", "3.8", "TLS", True, time.time(), time.time(), "high")
            for i in range(3)
        ]
...
class TestVNCConsoleInputOutput:
    ...
    async def test_get_clipboard_text(self, mock_vnc_manager):
        ...
        mock_vnc_manager.get_clipboard_text = AsyncMock(return_value="clipboard content")
